...
const Footer = () => {
  const icons = [
    ...
    { icon: <BsTwitch className="w-4 h-4" /> },
    { icon: <BsLinkedin className="w-4 h-4" /> },
    { icon: <BsYoutube className="w-4 h-4" /> },
    { icon: <BsDiscord className="w-4 h-4" /> },
    { icon: <BsGithub className="w-4 h-4" /> },
  ];

  return (
    <div
      className="h-fit w-screen flex flex-col items-center justify-end"
      id="community"
    >
      ...
      <div className="w-full h-fit bg-white flex justify-between items-center pl-6 sm:pl-12 md:pl-32 mt-10 pb-5 sm:pb-20">
        <div className="w-1/4 h-full">
          ...
          <ul className="flex p-0 m-0 text-xs justify-start items-center w-full">
            {icons.map((i: any, index: number) => {
              return (
                <li
                  key={index}
                  className="mr-2 sm:mr-3 flex justify-center items-center"
                >
                  {i.icon}
                </li>
              );
        ...
        <div className="flex w-2/4 h-full pr-8 sm:pr-16">
          <div className="w-1/4 mr-12 sm:mr-0">
            <ul className="m-0 p-0">
              <li className="font-bold mb-6 text-xs sm:text-sm sm:tracking-wider">
                Marketplace
              </li>
